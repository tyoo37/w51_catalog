...
filternames = filternames_narrow = all_filternames = [ 'f560w', 'f770w', 'f1000w', 'f1280w', 'f2100w']
...
def merge_daophot(module='nrca', detector='', daophot_type='basic', desat=False, bgsub=False, epsf=False, blur=False, target='brick',
    ...
    imgfns = [x
              for filn in filternames
              for x in glob.glob(f"{basepath}/{filn.upper()}/pipeline/jw0{filter_to_project[filn.lower()]}*mirimage_i2d.fits")
              if f'{module}_' in x or f'{module}1_' in x
             ]
