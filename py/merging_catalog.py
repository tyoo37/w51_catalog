...
def sanity_check_individual_table(tbl):
    ...
    instrument = 'NIRCam' if wavelength < 550 else 'MIRI'
    zeropoint = u.Quantity(jfilts.loc[f'JWST/{instrument}.{filtername.upper()}']['ZeroPoint'], u.Jy)
...
def combine_singleframe(tbls_, max_offset=0.10 * u.arcsec, realign=False, nanaverage=nanaverage_dask,
    ...
    if offsets_table is not None:
        tbls_ = [shift_individual_catalog(tbl, offsets_table, verbose=verbose) for tbl in tbls_]
    def _deduplicate_close_rows(tbl, separation_threshold):
        coords = tbl['skycoord_centroid'] if 'skycoord_centroid' in tbl.colnames else tbl['skycoord']
        i1, i2, sep2d, _ = coords.search_around_sky(coords, separation_threshold)
        ...
        # Prefer rows with more valid matches, then smaller propagated flux error
        nmatch_good = tbl['nmatch_good'] if 'nmatch_good' in tbl.colnames else np.zeros(len(tbl), dtype=int)
        ferr_col = tbl['flux_err'] if 'flux_err' in tbl.colnames else np.full(len(tbl), np.inf)
        # extract plain numpy array in case it's an astropy Quantity
        if hasattr(ferr_col, 'value'):
            ferr = np.array(ferr_col.value, dtype=float)
        else:
            ferr = np.array(ferr_col, dtype=float)

        to_remove = set()
        for a, b in zip(i1, i2):
            if a in to_remove or b in to_remove:
                continue
            if nmatch_good[a] > nmatch_good[b]:
                to_remove.add(b)
            elif nmatch_good[b] > nmatch_good[a]:
                to_remove.add(a)
            else:
                # tie-break: keep smaller propagated flux error
                if np.nan_to_num(ferr[a], nan=np.inf) <= np.nan_to_num(ferr[b], nan=np.inf):
                    to_remove.add(b)
                else:
                    to_remove.add(a)
        ...
        print(f"Removed {len(to_remove)} close-neighbor duplicates within {separation_threshold}.")
        return tbl[keep]
    tbls = []
    for tbl in tbls_:
        tbl = _deduplicate_close_rows(tbl, separation_threshold=0.5*min_offset)
        tbls.append(tbl)
...
def merge_catalogs(tbls, catalog_type='crowdsource', module='nrca',
                   ...
                   min_nmatch_narrow=4,nmatchcut=0,
    ...
    if nmatchcut > 0:
        for tbl in tbls:
            tbl = tbl[tbl['nmatch'] > nmatchcut]
            print(f"After nmatchcut={nmatchcut}, filter {tbl.meta['filter']} has {len(tbl)} rows")
...
def merge_individual_frames(module='merged', suffix="", desat=False, filtername='f410m', 
    ...
    tblfns = [x
              ...
              for x in glob.glob(f"{basepath}/{filtername.upper()}/"
                                 f"{filtername.lower()}_{module_}_visit{visitid:03d}*exp{exposure:05d}{desat}{bgsub}{fitpsf}{blur_}"
    ...
    # use pixel_scale for max_offset in combine_singleframe, which is used to determine whether to keep sources that are not mutual matches
    pix_scale = tb.meta['pixscale_as']
    print(f"Using pixel scale of 0.5*{pix_scale} arcsec for max_offset in combine_singleframe", flush=True)
    merged_exposure_table = combine_singleframe(tables, offsets_table=offsets_table, max_offset=1.0*pix_scale*u.arcsec, min_offset=1.0*pix_scale*u.arcsec)
...
def merge_crowdsource(module='nrca', suffix="", desat=False, bgsub=False,
    ...
    imgfns = [x
              ...
              for x in glob.glob(f"{basepath}/{filn.upper()}/pipeline/"
                                 f"jw0{obsid}-o002_t001_miri*{filn.lower()}*{module}_i2d.fits")
...
def merge_daophot(module='nrca', detector='', daophot_type='basic', desat=False, bgsub=False, epsf=False, blur=False, target='brick',
    ...
    nircam_filters = ['f140m', 'f150w', 'f162m', 'f182m', 'f187n', 'f210m', 'f335m', 'f360m', 'f410m', 'f405n', 'f480m']
    ...
    filter_to_project = {vv: key for key, val in obs_filters[f'{target}'].items() for vv in val}

    imgfns = []
    for filn in filternames:
        if filn in nircam_filters:
            glob_names = f"{basepath}/{filn.upper()}/pipeline/jw0{filter_to_project[filn.lower()]}*{filn.lower()}-{module}_i2d.fits"
            instrument = 'NIRCam'
        else:
            glob_names = f"{basepath}/{filn.upper()}/pipeline/jw0{filter_to_project[filn.lower()]}*{filn.lower()}_i2d.fits"
            instrument = 'MIRI'
        for x in glob.glob(glob_names):
            imgfns.append(x)
...
def main():
    ...
    parser.add_option("--filternames", dest="filternames",
                  default='f560w,f770w,f1000w,f1280w,f2100w',
                  help="Comma-separated filter names", metavar="filternames")
    (options, args) = parser.parse_args()

    modules = options.modules.split(",")

    filternames = filternames_narrow = all_filternames = options.filternames.split(",")
    obs_filters = {'brick': {'2221': filternames,
                            '1182': ['f444w', 'f356w', 'f200w', 'f115w'],
                            },
                'cloudc': {'2221': filternames},
    ...
    # Using the 'brick' keyword here makes it work for now, need to figure out how to
    # refactor it in cases where there are more filters available for other targets!
    filter_to_project = {vv: key for key, val in obs_filters[f'{options.target}'].items() for vv in val}
                        ...
                        for blur in (False, True):
                                    ...
                                    for filtername in (obs_filters[target][progid]):
                                        ...
                                        for method in indiv_merge_methods:
                                            ...
                                            suffix = {'crowdsource': '_nsky0',
                                                      ...
                                                      'dao_after_merger': '_combined_with_satstars'
                                                      }[method]
                            ...
                            if not options.skip_daophot:
                                ...
                                try:
                                    ...
                                    print(f"Running daophot with daophot_type='after_merger_combined'")
                                    merge_daophot(daophot_type='after_merger_combined', module=module, desat=desat, bgsub=bgsub, epsf=epsf,
                                                  target=target, basepath=basepath, blur=blur, indivexp=options.merge_singlefields, filternames=filternames)
                                
                                except Exception as ex:
                                    raise ex
                                try:
                                    print(f'daophot basic {module} desat={desat} bgsub={bgsub} epsf={epsf} blur={blur} fitpsf={fitpsf} target={target}', flush=True)
                                    print(f"Running daophot with daophot_type='basic'")
